...
// ============================================
// 2. Tool function: Convert string to uppercase
// ============================================
...
// ============================================
// 5. File reading: Parse FASTQ file
// ============================================
...
// ============================================
// 6. Core processing: Multi-threaded version
// ============================================
struct InsertResult {
    std::string insertSeq;
    std::string insertPro;
    int insertLen;
};
...
void processBatch(const std::vector<std::string>& reads,
    ...
    for (size_t i = 0; i < reads.size(); i++) {
        ...
        InsertResult& result = results[startIdx + i];
...
std::vector<InsertResult> findInsertSeq_parallel(
    ...
    size_t n = reads.size();
    std::vector<InsertResult> results(n);
    
    // Start multiple threads to process batches of reads
    std::vector<std::thread> threads;
    size_t batchSize = n / nThreads;
    
    for (unsigned int t = 0; t < nThreads; t++) {
        size_t start = t * batchSize;
        size_t end = (t == nThreads - 1) ? n : start + batchSize;
        
        // Extract data for the current batch
        std::vector<std::string> batch(reads.begin() + start, reads.begin() + end);
        
        // Start thread to process the batch
        threads.emplace_back(processBatch, std::move(batch), 
                           std::ref(seq1), std::ref(seq2), 
                           start, std::ref(results));
    }
...
void writeCSV(const std::unordered_map<std::string, int>& data,
    ...
    // Convert to a vector and sort by Quantity in descending order.
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4 || argc > 6) {
        std::cerr << "Usage: " << argv[0] 
                << " <fastq file> <sequence1> <sequence2> <output prefix> [number of threads]" << std::endl;
        return 1;
    }
    ...
    if (argc == 6) {
        nThreads = std::stoi(argv[5]);
    }
    
    std::cout << "=============================================" << std::endl;
    std::cout << "FASTQ Insert Sequence Analysis Tool v2.0 (Multi-threaded)" << std::endl;
    std::cout << "=============================================" << std::endl;
    ...
    // 2. Multi-threaded analysis of insert sequences
    std::cout << "[2/5] Analyzing insert sequences (" << nThreads << " threads)..." << std::endl;
    std::vector<InsertResult> results = findInsertSeq_parallel(reads, sequence1, sequence2, nThreads);
    
    // 3. Extract various types of data
    ...
    std::cout << "[5/5] Completed! Total time: " << duration / 1000.0 << " seconds" << std::endl;
    std::cout << "=============================================" << std::endl;
    ...
    std::cout << "  Not matched: " << notMatchCount << std::endl;
